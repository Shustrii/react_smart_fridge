				...
				<Form noValidate validated={validated} onSubmit={handleSubmit}>
					<Form.Group className='mb-3'>
						...
						<Form.Control required name='name' type='text' 
							placeholder='Введіть назву' defaultValue={product.name}
					...
					<Form.Group className='mb-3' controlId='formBasicPassword'>
						...
						<Form.Select required id='prtype' name='type' 
							aria-label='Default select example' 
							value={product.pr_type_id}
							onChange={(e) =>{
								//const prtName = productTypes.find(prType => prType.id === parseInt(e.target.value)).map((prt)=>{return(prt.name)}).toString()
								const prtName = productTypes.filter(prType => 
									prType.id === parseInt(e.target.value)).map((prt)=>{return(prt.name);}).toString();
